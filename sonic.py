...
def get_p_f_vectorized(
    ...
    workers : int, keyword-only, optional
        Number of parallel processes to use. Default is 1 (no parallelization).
        Supply -1 to use all available processors. In particular, `workers` is
        passed as `max_workers` to `concurrent.futures.ProcessPoolExecutor`.
