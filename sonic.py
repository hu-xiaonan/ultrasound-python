...
def get_p_f_vectorized(
    ...
    precalculated_geometry = _get_geometry_results(ula, points, subelement_num)
    p_f_arr = []
    print(f'Calculating pressure field.')
    if workers == 1:
        for f in tqdm(freqs):
            p_f_arr.append(
                _get_p_f(
        ...
        with ProcessPoolExecutor(workers) as executor:
            ...
            for p_f in tqdm(iterator, total=len(freqs)):
                p_f_arr.append(p_f)
...
def get_p_t(
    ...
    p_f_arr[idx] = get_p_f_vectorized(
        ...
        subelement_num=subelement_num, workers=workers,
    )
...
def get_p_scattered_f_vectorized(
    ...
    r = cdist(points, scatterers)
    p_scattered_f_arr = []
    print(f'Calculating scattered pressure field.')
    if workers == 1:
        ...
        with ProcessPoolExecutor(workers) as executor:
            ...
            for p_scattered_f in tqdm(iterator, total=len(freqs)):
...
def get_signals_rx_t(
    ...
    return get_p_scattered_t(
        ...
        db_thresh=db_thresh, subelement_num=subelement_num, workers=workers,
    ).real
